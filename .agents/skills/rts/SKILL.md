...
line count. This balances rule 8: split a _file_ when it grows, but do not
shatter a _function_ to get there.
...
function getShipping(order: Order) {
...
Do not abstract on the first sight of similarity. Code that merely _looks_ alike
...
parser) can be shared straight away — the rule targets _speculative_ abstraction,
